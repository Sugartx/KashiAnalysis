...
public class KashiCrawler {
    ...
    public List<String> getKashiList(SelectTypes select, String keyword, FuzzyTypes fuzzy){
        ...
        return kashiUrlList.stream().map(this::getKashi).collect(Collectors.toList());
    }

    private String getKashi(String kashiUrl){
        try {
            URL url = new URL(BASE_URL + kashiUrl);
            Document doc = Jsoup.parse(url, 10000);
            return doc.getElementById("kashi_area").text();
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        List<Map<Integer, List<WordFrequency>>> resultMapList = kachiList.stream().map(WordFrequencyAnalysis::analysis).collect(Collectors.toList());
        resultMapList.forEach(WordFrequencyAnalysis::print);
    }
}
